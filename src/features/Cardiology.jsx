...
const TAG_STYLES = {
  Interventional: "bg-rose-50 text-rose-700 ring-rose-200",
  Electrophysiology: "bg-indigo-50 text-indigo-700 ring-indigo-200",
  "Heart Failure": "bg-amber-50 text-amber-700 ring-amber-200",
  Pediatric: "bg-teal-50 text-teal-700 ring-teal-200",
  Preventive: "bg-emerald-50 text-emerald-700 ring-emerald-200",
  "Cardiac Surgery": "bg-slate-100 text-slate-700 ring-slate-300",
};
...
const HOSPITALS = [
  "Ashoka Heart Institute",
  "Sunrise Cardiac Center",
  "St. Luke's Medical",
  "Metro Cardiology Group",
  "Orchid Heart Hospital",
  "Pinewood Clinic",
...
const DOCTORS = Array.from({ length: 24 }, (_, i) => {
  ...
  const gender = i % 3 === 0 ? "Dr." : "Dr.";
  const available = rand() > 0.35;
  return {
    id: i + 1,
    name: `${gender} ${FIRST_NAMES[i]} ${LAST_NAMES[(i * 5) % LAST_NAMES.length]}`,
    subspecialty,
    experience,
    rating,
    hospital,
...
function Initials({ name }) {
  ...
  return (
    <div className="flex h-14 w-14 flex-none items-center justify-center rounded-full bg-gradient-to-br from-slate-800 to-slate-950 text-base font-semibold text-white ring-4 ring-rose-100">
...
function PulseDivider() {
  return (
    <svg
      viewBox="0 0 200 24"
      className="h-5 w-full text-rose-300"
...
export default function CardiologyDoctors() {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex flex-col gap-1">
          ...
          <p className="max-w-xl text-slate-500">
            24 cardiologists across interventional, electrophysiology, heart
            failure, pediatric, preventive and surgical care.
          </p>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-sm outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100 sm:max-w-xs"
          />

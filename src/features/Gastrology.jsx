...
const TAG_STYLES = {
  "General Gastroenterology": "bg-emerald-50 text-emerald-700 ring-emerald-200",
  Hepatology: "bg-amber-50 text-amber-700 ring-amber-200",
  "Endoscopy & IBD": "bg-teal-50 text-teal-700 ring-teal-200",
  "Pancreatic Disorders": "bg-rose-50 text-rose-700 ring-rose-200",
  "Colorectal Care": "bg-indigo-50 text-indigo-700 ring-indigo-200",
  "Pediatric GI": "bg-slate-100 text-slate-700 ring-slate-300",
};
...
const HOSPITALS = [
  "Ashoka Digestive Care Institute",
  "Sunrise GI Center",
  "St. Luke's Medical",
  "Metro Gastroenterology Group",
  "Orchid Liver & Gut Hospital",
  "Pinewood GI Clinic",
...
const DOCTORS = Array.from({ length: 12 }, (_, i) => {
  const rand = seededRandom(i * 83 + 31);
...
function Initials({ name }) {
  ...
  return (
    <div className="flex h-14 w-14 flex-none items-center justify-center rounded-full bg-gradient-to-br from-slate-800 to-slate-950 text-base font-semibold text-white ring-4 ring-emerald-100">
...
function PulseDivider() {
  return (
    <svg
      viewBox="0 0 200 24"
      className="h-5 w-full text-emerald-300"
...
function DoctorCard({ doc }) {
  return (
    <div className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <BookingButton doctor={{ ...doc, specialty: "Gastroenterologist", fee: "₹1,600" }} className="mt-4 w-full rounded-xl bg-slate-900 py-2.5 text-sm font-semibold text-white transition group-hover:bg-emerald-600">Book appointment</BookingButton>
    </div>
  );
}
...
export default function GastroenterologyDoctors() {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex flex-col gap-1">
          <span className="text-sm font-semibold uppercase tracking-wide text-emerald-600">
            Gastroenterology directory
          </span>
          <h1 className="text-3xl font-bold text-slate-900">
            Find a gastroenterology specialist
          </h1>
          <p className="max-w-xl text-slate-500">
            12 gastroenterologists across hepatology, endoscopy and IBD,
            pancreatic disorders, colorectal care, and pediatric GI.
          </p>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-sm outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 sm:max-w-xs"
          />

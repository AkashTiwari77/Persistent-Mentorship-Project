...
const TAG_STYLES = {
  "Stroke & Vascular": "bg-rose-50 text-rose-700 ring-rose-200",
  Epilepsy: "bg-indigo-50 text-indigo-700 ring-indigo-200",
  "Movement Disorders": "bg-amber-50 text-amber-700 ring-amber-200",
  Neuromuscular: "bg-teal-50 text-teal-700 ring-teal-200",
  "Pediatric Neurology": "bg-emerald-50 text-emerald-700 ring-emerald-200",
  "Headache Medicine": "bg-slate-100 text-slate-700 ring-slate-300",
};
...
const HOSPITALS = [
  "Ashoka Neuro Institute",
  "Sunrise Neuroscience Center",
  "St. Luke's Medical",
  "Metro Neurology Group",
  "Orchid Brain & Spine Hospital",
  "Pinewood Clinic",
...
const DOCTORS = Array.from({ length: 18 }, (_, i) => {
  const rand = seededRandom(i * 41 + 7);
  ...
  return {
    ...
    hospital,
    available,
    patients: 400 + Math.floor(rand() * 3200),
  };
});
...
function Initials({ name }) {
  ...
  return (
    <div className="flex h-14 w-14 flex-none items-center justify-center rounded-full bg-gradient-to-br from-slate-800 to-slate-950 text-base font-semibold text-white ring-4 ring-indigo-100">
...
function PulseDivider() {
  return (
    <svg
      viewBox="0 0 200 24"
      className="h-5 w-full text-indigo-300"
...
function DoctorCard({ doc }) {
  return (
    <div className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <BookingButton doctor={{ ...doc, specialty: "Pediatrician", fee: "₹1,400" }} className="mt-4 w-full rounded-xl bg-slate-900 py-2.5 text-sm font-semibold text-white transition group-hover:bg-indigo-600">Book appointment</BookingButton>
    </div>
  );
}
...
export default function NeurologyDoctors() {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex flex-col gap-1">
          ...
          <p className="max-w-xl text-slate-500">
            18 neurologists across stroke care, epilepsy, movement disorders,
            neuromuscular medicine, pediatric neurology and headache care.
          </p>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 sm:max-w-xs"
          />
